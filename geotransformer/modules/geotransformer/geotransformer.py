...
class GeometricStructureEmbedding(nn.Module):
    ...
    def get_embedding_indices(self, points):
        ...
        if ( num_point <= num_k ):
            ...
            knn_indices_num = dist_map.topk(k=num_k + 1, dim=2, largest=False)[1][:, :, 1:]
            d_indices = torch.gather(d_indices, dim=2, index=knn_indices_num)
            knn_indices_num = knn_indices_num.unsqueeze(3).expand(batch_size, num_point, num_k, 3)  # (B, N, num_k, 3)
            anc_vectors_num = torch.gather(expanded_points, dim=2, index=knn_indices_num) - points.unsqueeze(2)  # (B, N, num_k , 3)
            ref_vectors = ref_vectors.unsqueeze(2).expand(batch_size, num_point, num_k, k, 3)  # (B, N, num_k, k, 3)
            anc_vectors = anc_vectors_num.unsqueeze(3).expand(batch_size, num_point, num_k, k, 3)  # (B, N, num_k, k, 3)
        sin_values = torch.linalg.norm(torch.cross(ref_vectors, anc_vectors, dim=-1), dim=-1)  # (B, N, N, k)
        cos_values = torch.sum(ref_vectors * anc_vectors, dim=-1)  # (B, N, N, k)
        angles = torch.atan2(sin_values, cos_values)  # (B, N, N, k)
        a_indices = angles * self.factor_a
    ...
    def forward(self, points):
        ...
        """
        lenlen1 = int(a_embeddings.shape[1]/3)
        lenlen2 = lenlen1*2
        lenlen3 = a_embeddings.shape[1]
        a_embeddings1 = a_embeddings[0][0:lenlen1]
        a_embeddings2 = a_embeddings[0][lenlen1:lenlen2]
        a_embeddings3 = a_embeddings[0][lenlen2:lenlen3]
        if self.reduction_a == 'max':
            a_embeddings1 = a_embeddings1.max(dim=2)[0]
            a_embeddings2 = a_embeddings2.max(dim=2)[0]
            a_embeddings3 = a_embeddings3.max(dim=2)[0]
        else:
            a_embeddings = a_embeddings.mean(dim=3)
        a_embeddings4 = torch.empty(1, a_embeddings.shape[1], a_embeddings.shape[2], a_embeddings.shape[4])
        a_embeddings4 = a_embeddings4.to('cuda')
        a_embeddings4[0][0:lenlen1] = a_embeddings1
        a_embeddings4[0][lenlen1:lenlen2] = a_embeddings2
        a_embeddings4[0][lenlen2:lenlen3] = a_embeddings3

        embeddings = d_embeddings + a_embeddings4"""
